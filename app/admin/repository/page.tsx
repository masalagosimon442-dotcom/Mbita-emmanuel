...
export default function RepositoryPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of Array.from(selected)) {
      await fetch(`/api/admin/repository?id=${id}`, { method: "DELETE" });
    }
    setSelected(new Set());
    ...
    for (const id of Array.from(selected)) {
      ...
      if (item) {
        ...
        await fetch("/api/admin/repository", {
          ...
          body: JSON.stringify({ id, title: item.title, description: item.description, repoUrl: item.repoUrl ?? "", demoUrl: item.demoUrl ?? "", language: item.language ?? "", stars: item.stars, tags: tags ? tags.split(",").map(t => t.trim()).filter(Boolean) : [], published }),
        });
      }
    }
    setSelected(new Set());
