    ...
    for (const id of Array.from(selected)) {
      ...
      if (item) {
        await fetch("/api/admin/presentations", {
          ...
          body: JSON.stringify({ id, title: item.title, event: item.event, date: item.date.split("T")[0], slideUrl: item.slideUrl ?? "", videoUrl: item.videoUrl ?? "", description: item.description ?? "", published }),
        });
      }
    }
    setSelected(new Set());
